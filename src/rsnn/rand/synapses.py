...
def new_synapses(
    ...
    Args:
        sources (np.ndarray): Array of source neuron indices.
    ...
    Returns:
        pl.DataFrame: Synapses DataFrame with columns 'source', 'target', 'delay', 'weight'.
...
def rand_synapses(
    ...
    Returns:
        pl.DataFrame: Synapses DataFrame with randomly connected neurons, zero-initialized weights, and random delays.

    Raises:
        ValueError: If n_neurons or n_synapses is negative.
        ValueError: If min_delay is not less than max_delay or if min_delay is negative.

    Notes:
        Synapses are initialized with zero weights and must be trained or set manually. No connectivity constraints are applied.
    """
    if n_neurons < 0:
        raise ValueError("n_neurons must be non-negative")

    if n_synapses < 0:
        raise ValueError("n_synapses must be non-negative")
    ...
    return new_synapses(
        sources=rng.integers(n_neurons, size=n_synapses, dtype=np.uint32),
        targets=rng.integers(n_neurons, size=n_synapses, dtype=np.uint32),
...
def rand_synapses_fc(
    ...
    Raises:
        ValueError: If n_neurons is negative.
        ValueError: If min_delay is not less than max_delay or if min_delay is negative

    Notes:
        Generates n_neurons² synapses including self-connections. Synapses are initialized with zero weights.
    """
...
def rand_synapses_fin(
    ...
    Returns:
        pl.DataFrame: Synapses DataFrame with fixed in-degree connectivity, zero-initialized weights, and random delays.

    Raises:
        ValueError: If n_neurons or n_synapses is negative or if n_synapses is not divisible by n_neurons.
        ValueError: If min_delay is not less than max_delay or if min_delay is negative.
...
def rand_synapses_fout(
    ...
    Args:
        ...
        min_delay (float): Minimum connection delay.
        max_delay (float): Maximum connection delay.
        rng (Optional[np.random.Generator], optional): Random number generator. If None, uses default_rng(). Defaults to None.

    Returns:
        pl.DataFrame: Synapses DataFrame with fixed out-degree connectivity, zero-initialized weights, and random delays.
    ...
    Notes:
        Each neuron makes exactly n_synapses/n_neurons outgoing connections. Synapses are initialized with zero weights.
    """
    if n_neurons < 0:
        raise ValueError("n_neurons must be non-negative")
...
def rand_synapses_fin_fout(
    ...
    Creates synaptic connections where each neuron has equal incoming and outgoing connections. Uses permutation to ensure balanced connectivity while maintaining randomness in the connection pattern.
    ...
    Returns:
        pl.DataFrame: Synapses DataFrame with balanced connectivity, zero-initialized weights, and random delays.
