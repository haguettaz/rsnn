...
#         self.x = self.model.addMVar(self.n_vars, lb=-np.inf, ub=np.inf, name="x")
...
#         # if not self.is_feas:
#         #     logger.warning(
#         #         "New constraints are incompatible with the current ones."
#         #     )
...
#         logger.debug(
#             f"{b.size} new constraint(s) successfully added. The new quadratic problem with {self.n_vars} variables and {self.n_cstrs} constraints is feasible."
#         )
...
#             for in_f_times_n, times_n, offset in zip(in_times, times, f_times):
...
#         """
#         A, b = [], []
#         self.weights = self.solver.model.x

#         for n in range(self.n_intervals):
#             # Potential constraints
#             self.z_c0[n] = (
#                 np.inner(self.z_ck0[n][:, :-1], self.weights)
#                 + self.z_ck0[n][:, -1]
#             )
#             self.z_c1[n] = (
#                 np.inner(self.z_ck1[n][:, :-1], self.weights)
#                 + self.z_ck1[n][:, -1]
#             )
...
#             self.dz_c0[n] = (
#                 np.inner(self.dz_ck0[n][:, :-1], self.weights)
#                 + self.dz_ck0[n][:, -1]
#             )
#             self.dz_c1[n] = (
#                 np.inner(self.dz_ck1[n][:, :-1], self.weights)
#                 + self.dz_ck1[n][:, -1]
#             )
#             dz_dvmax, dz_imax, dz_dtmax = find_maximum_violation(
#                 self.dz_c0[n], self.dz_c1[n], self.dz_length[n], self.dz_lim[n] + FEAS_TOL
#             ) or (0.0, None, None)
...
#                 logger.info(
#                     f"Learning succeeds! Solved in {i+1} refinement iterations. Number of variables: {self.solver.n_vars}. Number of constraints: {self.solver.n_cstrs}. Cost: {self.solver.model.objVal:.6f}."
#                 )
