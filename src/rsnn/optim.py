...
#             ).alias("start"),
#             pl.col("in_index"),
#             pl.lit(0.0, pl.Float64).alias("weight"),
...
#     v_states = (
#         in_states.filter(
#             pl.col("in_index").is_not_null() | (pl.col("weight").abs() > 0.0)
#         )
#         .select("f_index", "in_index", "weight")
#         .unique()
...
#     states = in_states.extend(v_states)

#     return out_spikes, synapses, states


# def init_energy_optimization(spikes, synapses):
#     out_spikes, synapses, states = init_offline_optimization(spikes, synapses)
#     states = scan_states(states, over=["f_index", "in_index"])
#     return out_spikes, synapses, states


# def init_continuous_optimization(spikes, synapses, eps):
#     """Returns states with transformed spikes and synapses."""
#     if eps < 0:
#         raise ValueError("epsilon must be positive.")
...
#     # active states
#     v_states = (
#         states.select("f_index", "in_index", "weight")
...
#                     * (2 * pl.col("coef_0") + pl.col("coef_1"))
#                     * (1 + 2 * pl.col("length"))
#                     + 2 * pl.col("coef_1") ** 2 * pl.col("length") ** 2
#                 )
#                 * (-2 * pl.col("length")).exp()
#             ).alias("energy"),
#         )
...
# def energy_optimization(spikes, synapses, wmin, wmax):
#     # Implement event-based offline optimization here
#     if wmin >= wmax:
#         raise ValueError("wmin must be less than wmax.")

#     out_spikes, synapses, states = init_energy_optimization(spikes, synapses)
...
#     # Objective function: minimize synaptic energy
#     metric_matrix = computes_syn_energy_metric_matrix(states, synapses)
#     model.setObjective(
#         weights @ metric_matrix @ weights,
#         sense=gp.GRB.MINIMIZE,
#     )

#     # Firing constraints
#     syn_lin_map, rec_lin_offset = compute_linear_map(
#         states, out_spikes.select("f_index", "neuron", "time"), synapses
#     )
#     model.addConstr(syn_lin_map @ weights + rec_lin_offset == FIRING_THRESHOLD)

#     syn_lin_map, rec_lin_offset = compute_linear_map(
#         states, out_spikes.select("f_index", "neuron", "time"), synapses, deriv=1
#     )
#     model.addConstr(syn_lin_map @ weights + rec_lin_offset >= 1e-9)  # optional
...
#     synapses = update_weights(synapses, weights.X)
#     return synapses
